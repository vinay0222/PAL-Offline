...
public class NormalTestAdapter extends RecyclerView.Adapter {
    ...
    public final ArrayList<HashMap<Integer, String>> questionAttamptedArrayList;
    private final HashMap<Integer, String> questionAttamptedHashMap;
    private final Global global;
    private int count = 0;

    public NormalTestAdapter(Context context, ArrayList<HashMap<Integer, String[]>> contentArrayList, ArrayList<HashMap<String, String>> questionsArrayList, RecyclerView mRecyclerView, String selectedLanguage, int height, String submit, String submitTest, String imageInternet, String lastMessage) {
    ...
    public boolean equals(Object o) {
        ...
        NormalTestAdapter that = (NormalTestAdapter) o;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        if (viewType == TYPE_ITEM) {
            ...
            return new NormalTestAdapter.ViewItem(view);
        }
        return null;
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolderValue, final int position) {
        if (viewHolderValue instanceof NormalTestAdapter.ViewItem) {
            final NormalTestAdapter.ViewItem viewHolder = (NormalTestAdapter.ViewItem) viewHolderValue;
            try {
...
//                }
                ...
                viewHolder.textViewSubmit.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        ((NormalTestActivity) context).finalSubmition();
                    }
                });
                ...
                viewHolder.textViewOption2.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        boolean exists = true;
                        if(!((NormalTestActivity)context).restoreQuestionHashMap.containsKey(position+"-"+2)){
                            optionTwoClick(position, "Text");
                            ((NormalTestActivity)context).restoreQuestionHashMap.put(position+"-"+2, true);
                            exists = false;
                        }
                        for (Map.Entry<String, Boolean> entry :((NormalTestActivity)context).restoreQuestionHashMap.entrySet()) {
                            String key = entry.getKey();
                ...
                viewHolder.textViewOption3.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        boolean exists = true;
                        if(!((NormalTestActivity)context).restoreQuestionHashMap.containsKey(position+"-"+3)){
                            optionThirdClick(position, "Text");
                            ((NormalTestActivity)context).restoreQuestionHashMap.put(position+"-"+3, true);
                            exists = false;
                        }
                        for (Map.Entry<String, Boolean> entry :((NormalTestActivity)context).restoreQuestionHashMap.entrySet()) {
                            String key = entry.getKey();
                ...
                viewHolder.textViewOption4.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        boolean exists = true;
                        if(!((NormalTestActivity)context).restoreQuestionHashMap.containsKey(position+"-"+4)){
                            optionFourthClick(position, "Text");
                            ((NormalTestActivity)context).restoreQuestionHashMap.put(position+"-"+4, true);
                            exists = false;
                        }
                        for (Map.Entry<String, Boolean> entry :((NormalTestActivity)context).restoreQuestionHashMap.entrySet()) {
                            String key = entry.getKey();
    ...
    private void moveToNextQuestion(final int position) {
        try {
            if (position + 1 != questionsArrayList.size()) {
                NormalTestAdapter.ViewItem viewHolder = (ViewItem) mRecyclerView.findViewHolderForAdapterPosition(position);
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        ((NormalTestActivity) context).scroll(position + 1);
                    }
                }, 700);
            } else {
                try {
                    ...
                    ((NormalTestActivity) context).checkItsLastQuestion();
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    private void restoreToOriginalQuestion(int position, String option) {
        ...
        trackArrayList.get(position).put("Status", "PENDING");
...
//        questionAttamptedArrayList.remove(removeAbleHasmap);
        ...
        ((NormalTestActivity) context).setTextOnQuestionAttampted(count);
        ((NormalTestActivity) context).setAttemptedQ(count);
        ((NormalTestActivity) context).setYetToAttemptQ(questionsArrayList.size() - count);
        ((NormalTestActivity) context).removeAttempt(position);
        ((NormalTestActivity) context).checkSkippedQ(position, 1);
        ((NormalTestActivity) context).refreshQuestionsListingAdapter();
//        ((NormalTestActivity) context).updateNextQuestionButtonText(1, position);
        notifyDataSetChanged();
    }
    ...
    private void optionOneClick(int position, String type) {
        ...
        ((NormalTestActivity) context).setAttemptedQ(count);
        ((NormalTestActivity) context).setYetToAttemptQ(questionsArrayList.size() - count);
        ((NormalTestActivity) context).attemptedQMap.add(position);
        ((NormalTestActivity) context).checkSkippedQ(position, 1);
        ((NormalTestActivity) context).refreshQuestionsListingAdapter();
//        ((NormalTestActivity) context).updateNextQuestionButtonText(0, position);
