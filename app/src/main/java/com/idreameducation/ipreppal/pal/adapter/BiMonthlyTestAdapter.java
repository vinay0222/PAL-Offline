...
public class BiMonthlyTestAdapter extends RecyclerView.Adapter {
    ...
    private ArrayList<HashMap<Integer, String[]>> contentArrayList;
    ...
    private ArrayList<HashMap<Integer, String>> questionAttamptedArrayList;
    private HashMap<Integer, String> questionAttamptedHashMap;
    private Global global;
    private int count = 0;

    public BiMonthlyTestAdapter(Context context, ArrayList<HashMap<Integer, String[]>> contentArrayList, ArrayList<HashMap<String, String>> questionsArrayList, RecyclerView mRecyclerView, String selectedLanguage, int height, String submit, String submitTest, String imageInternet, String lastMessage) {
    ...
    public boolean equals(Object o) {
        ...
        BiMonthlyTestAdapter that = (BiMonthlyTestAdapter) o;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        if (viewType == TYPE_ITEM) {
            ...
            return new BiMonthlyTestAdapter.ViewItem(view);
        }
        return null;
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolderValue, final int position) {
        if (viewHolderValue instanceof BiMonthlyTestAdapter.ViewItem) {
            final BiMonthlyTestAdapter.ViewItem viewHolder = (BiMonthlyTestAdapter.ViewItem) viewHolderValue;
            try {
...
//                    Glide.with(context).load(questionsArrayList.get(position).get("image")).into(viewHolder.questionImageView);
                        Glide.with(context)
                                .load(questionsArrayList.get(position).get("image"))
...
//                viewHolder.textViewSubmit.setVisibility(View.VISIBLE);
                ...
                viewHolder.textViewSubmit.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        ((BiMonthlyTestActivity) context).finalSubmition();
                    }
                });
                viewHolder.textViewOption1.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        boolean exists = true;
                        if(!((BiMonthlyTestActivity)context).restoreQuestionHashMap.containsKey(position+"-"+1)){
                            optionOneClick(position, "Text");
                            ((BiMonthlyTestActivity)context).restoreQuestionHashMap.put(position+"-"+1, true);
                            exists = false;
                        }
                        for (Map.Entry<String, Boolean> entry :((BiMonthlyTestActivity)context).restoreQuestionHashMap.entrySet()) {
                            ...
                            if(key.equals(position+"-"+1)){
                                if(exists){
                                    if(value){
                                        ...
                                        if(((BiMonthlyTestActivity)context).pos == questionsArrayList.size()){
                                            ((BiMonthlyTestActivity)context).pos = ((BiMonthlyTestActivity)context).pos - 1;
                                        }
                                    }else{
                                        optionOneClick(position, "Text");
                                        entry.setValue(true);
                ...
                viewHolder.textViewOption2.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        boolean exists = true;
                        if(!((BiMonthlyTestActivity)context).restoreQuestionHashMap.containsKey(position+"-"+2)){
                            optionTwoClick(position, "Text");
                            ((BiMonthlyTestActivity)context).restoreQuestionHashMap.put(position+"-"+2, true);
                            exists = false;
                        }
                        for (Map.Entry<String, Boolean> entry :((BiMonthlyTestActivity)context).restoreQuestionHashMap.entrySet()) {
                            String key = entry.getKey();
                ...
                viewHolder.textViewOption3.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        boolean exists = true;
                        if(!((BiMonthlyTestActivity)context).restoreQuestionHashMap.containsKey(position+"-"+3)){
                            optionThirdClick(position, "Text");
                            ((BiMonthlyTestActivity)context).restoreQuestionHashMap.put(position+"-"+3, true);
                            exists = false;
                        }
                        for (Map.Entry<String, Boolean> entry :((BiMonthlyTestActivity)context).restoreQuestionHashMap.entrySet()) {
                            String key = entry.getKey();
                ...
                viewHolder.textViewOption4.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        boolean exists = true;
                        if(!((BiMonthlyTestActivity)context).restoreQuestionHashMap.containsKey(position+"-"+4)){
                            optionFourthClick(position, "Text");
                            ((BiMonthlyTestActivity)context).restoreQuestionHashMap.put(position+"-"+4, true);
                            exists = false;
                        }
                        for (Map.Entry<String, Boolean> entry :((BiMonthlyTestActivity)context).restoreQuestionHashMap.entrySet()) {
                            String key = entry.getKey();
                        ...
                        switch (i) {
                            case 0:
                                if (questionsArrayList.get(position).get("type1").equalsIgnoreCase("Text")) {
                                    ...
                                    viewHolder.textViewOption2.setCompoundDrawablesWithIntrinsicBounds(R.mipmap.b_option, 0, 0, 0);
                                    viewHolder.textViewOption3.setCompoundDrawablesWithIntrinsicBounds(R.mipmap.c_option, 0, 0, 0);
                                    viewHolder.textViewOption4.setCompoundDrawablesWithIntrinsicBounds(R.mipmap.d_option, 0, 0, 0);
                            ...
                            case 1:
                                ...
                                if (questionsArrayList.get(position).get("type1").equalsIgnoreCase("Text")) {
                                    ...
                                    viewHolder.textViewOption1.setCompoundDrawablesWithIntrinsicBounds(R.mipmap.a_option, 0, 0, 0);
            ...
            public void run() {
                try {
                    if (position + 1 != questionsArrayList.size()) {
                        BiMonthlyTestAdapter.ViewItem viewHolder = (ViewItem) mRecyclerView.findViewHolderForAdapterPosition(position);

//                        ((BiMonthlyTestActivity) context).scroll(position + 1);
                    } else {
                        try {
                            BiMonthlyTestAdapter.ViewItem viewHolder = (ViewItem) mRecyclerView.findViewHolderForAdapterPosition(position);
                           // ((BiMonthlyTestActivity) context).scroll(position + 1);
...
//        HashMap<Integer, String> removeAbleHasmap = questionAttamptedArrayList.get(position);
//        questionAttamptedArrayList.remove(removeAbleHasmap);
        ...
        ((BiMonthlyTestActivity) context).setTextOnQuestionAttampted(count);
        ((BiMonthlyTestActivity) context).setAttemptedQ(count);
        ((BiMonthlyTestActivity) context).setYetToAttemptQ(questionsArrayList.size() - count);
        ((BiMonthlyTestActivity) context).removeAttempt(position);
        ((BiMonthlyTestActivity) context).checkSkippedQ(position, 1);
        ((BiMonthlyTestActivity) context).refreshQuestionsListingAdapter();
        ((BiMonthlyTestActivity) context).updateNextQuestionButtonText(1, position);
        notifyDataSetChanged();
    }
    ...
    private void optionOneClick(int position, String type) {
        ...
        ((BiMonthlyTestActivity) context).setAttemptedQ(count);
        ((BiMonthlyTestActivity) context).setYetToAttemptQ(questionsArrayList.size() - count);
        ((BiMonthlyTestActivity) context).attemptedQMap.add(position);
        ((BiMonthlyTestActivity) context).checkSkippedQ(position, 1);
        ((BiMonthlyTestActivity) context).refreshQuestionsListingAdapter();
        ((BiMonthlyTestActivity) context).updateNextQuestionButtonText(0, position);
...
//        }
    ...
    public class ViewItem extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        private RelativeLayout reletiveLayoutImage;
        private TextView NoInternetConnectionTextView;
        private TextView textViewReadMore;
        private TextView textViewLastQuestionMessage;
