...
public class ReviewModelTestPaperActivity extends AppCompatActivity {
    ...
    private void getLastSubmitedTestPaper() throws Exception {
        Util.showDialog(context);
        global.getDatabaseReference().child(ApplicationConstants.USER_DATA).child(Util.getUserId(context)).child(Util.getSelectedBoardName(context)).child(Util.getSelectedClassName(context)).child(Util.getSubject(context)).child(ApplicationConstants.MODEL_TEST_PAPER).child(Util.getTopicID(context) + ":" + Util.getTopicNameAlt(context)).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                try {
                    ...
                    try {
                        getDetailsLastSubmitedTestPaperScore(key);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                } catch (Exception e) {
    ...
    private void getDetailsLastSubmitedTestPaper(String key) throws Exception {
        global.getDatabaseReference().child(ApplicationConstants.USER_DATA).child(Util.getUserId(context)).child(Util.getSelectedBoardName(context)).child(Util.getSelectedClassName(context)).child(Util.getSubject(context)).child(ApplicationConstants.MODEL_TEST_PAPER).child(Util.getTopicID(context) + ":" + Util.getTopicNameAlt(context)).child(key).child(ApplicationConstants.DETAILS).addValueEventListener(new ValueEventListener() {
    ...
    private void getDetailsLastSubmitedTestPaperScore(final String key) throws Exception {
        global.getDatabaseReference().child(ApplicationConstants.USER_DATA).child(Util.getUserId(context)).child(Util.getSelectedBoardName(context)).child(Util.getSelectedClassName(context)).child(Util.getSubject(context)).child(ApplicationConstants.MODEL_TEST_PAPER).child(Util.getTopicID(context) + ":" + Util.getTopicNameAlt(context)).child(key).child(ApplicationConstants.SCORES).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                try {
                    Util.dismissDialog();
                    HashMap<String, String> scoresHashmap = (HashMap<String, String>) dataSnapshot.getValue();
                    setStaticText(scoresHashmap, key);
    ...
    private void setStaticText(final HashMap<String, String> scoresHashmap, String key) {
